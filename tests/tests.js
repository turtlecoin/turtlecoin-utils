...
/* Standardjs is trash lol https://github.com/standard/standard/issues/865 */
;(async () => {
  ...
  const outputMnemonic = await cnUtil.createAddressFromSeed(rawSeed)
  const outputSeed = await cnUtil.createAddressFromMnemonic(rawMnemonic)
  ...
  const outputAddress = await cnUtil.encodeRawAddress(testAddressRaw)
  const outputRaw = await cnUtil.decodeAddress(testAddress)
  ...
  const newAddress = await cnUtil.createNewAddress(testAddress, 'english')
  const newAddressByKey = await cnUtil.createAddressFromKeys(newAddress.spend.privateKey, newAddress.view.privateKey)
  ...
  const derivedPublicKey = await cnUtil.privateKeyToPublicKey(testPrivateKey)
  ...
  const mnemonicAddressByKey = await cnUtil.createAddressFromKeys(outputSeed.spend.privateKey, outputSeed.view.privateKey)
  ...
  const nonMnemonicAddressByKey = await cnUtil.createAddressFromKeys(nonMnemonicPrivateSpendKey, nonMnemonicPrivateViewKey)
  ...
  var transfers = await cnUtil.createTransactionOutputs(newAddress.address, amount)
  var amounts = []
  ...
  console.log('')
  console.log('Creating outputs for amount %s to %s', amount, newAddress.address)
  transfers = await cnUtil.createTransactionOutputs(newAddress.address, amount)
  amounts = []
  transfers.forEach((elem) => {
    amounts.push(elem.amount)
  })
  ...
  const newAthenaAddress = await cnUtil.encodeAddress(newAddress.view.publicKey, newAddress.spend.publicKey, false, athenaPrefix.decimal)
  const newAthenaAddressByKey = await cnUtil.createAddressFromKeys(newAddress.spend.privateKey, newAddress.view.privateKey, athenaPrefix.decimal)
  ...
  const derivation = await cnUtil.generateKeyDerivation(txPublicKey, walletPrivateViewKey)

  const ourOutputIndex = 2

  /* (First output) This is not our output. */
  const publicSpendKey1 = await cnUtil.underivePublicKey(derivation, 0, 'aae1b90b4d0a7debb417d91b7f7aa8fdfd80c42ebc6757e1449fd1618a5a3ff1')
  ...
  const publicSpendKey2 = await cnUtil.underivePublicKey(derivation, ourOutputIndex, 'bb55bef919d1c9f74b5b52a8a6995a1dc4af4c0bb8824f5dc889012bc748173d')
  ...
  const [keyImage] = await cnUtil.generateKeyImage(txPublicKey, walletPrivateViewKey, walletPublicSpendKey, walletPrivateSpendKey, ourOutputIndex)
  const [keyImagePrimitive] = await cnUtil.generateKeyImagePrimitive(walletPublicSpendKey, walletPrivateSpendKey, ourOutputIndex, derivation)
